...
== Configuration
  ...
  Set global configuration if you want:

  Bitly.config do |c|
    c.login = 'login-here'
    c.api_key = 'key-here'
    c.key = 'key-here' # deprecated
  ...
  Bitly.key = 'key-here' # deprecated
  Bitly.use_ssl = false # default: true
...
== Proxy
  All calls will use the proxy specified by environment variable "http_proxy".


== SSL
  All calls will use SSL by default. If you want to disable:
